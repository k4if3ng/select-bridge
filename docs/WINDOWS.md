...
- “查询目标”使用单选项在 Goldendict-ng 弹窗与已保存的自定义 URL 之间切换；自定义模板为空或无效时该单选项禁用；
- URL 编辑窗口与快捷键捕获窗口互斥，同一时间只允许一个设置类窗口；重复打开会激活现有窗口；
...
- “高级”子菜单提供打开配置文件、打开配置目录和原子重新加载。重新加载失败时继续使用旧状态；
...
v1.2.0 修复 Windows native UI 中文编码，新增托盘查询目标与自定义 URL 编辑入口、schema 9 配置迁移和严格重载，并为 x64/ARM64 Release 及 Pull Request CI 提供独立构建任务。
